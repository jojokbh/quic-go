...
func (c *client) dial() error {
	...
	if len(c.multi) > 0 {
		println("Dial multi")
		c.session, err = dialMultiAddr(c.hostname, c.multi, c.tlsConf, c.ifat, c.config)
		c.multiSession = true
	} else {
		c.multiSession = false
		if c.dialer != nil {
...
// RoundTrip executes a request and returns a response
func (c *client) RoundTripMulti(req *http.Request) (*http.Response, error) {
	if req.URL.Scheme != "https" {
		return nil, errors.New("http3: unsupported scheme")
	}

	if c.multi != "" {
		return nil, errors.New("No multicast addr")
	}
	...
	//c.multiSession = c.dialMulti(c.multi, c.ifat)
	// Immediately send out this request, if this is a 0-RTT request.
	if req.Method == MethodGet0RTT {
		req.Method = http.MethodGet
	} else {
