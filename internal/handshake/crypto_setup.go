...
func (h *cryptoSetup) SetReadKey(encLevel qtls.EncryptionLevel, suite *qtls.CipherSuiteTLS13, trafficSecret []byte) {
	...
	case qtls.Encryption0RTT:
		...
		h.mutex.Unlock()
		h.logger.Debugf("Installed 0-RTT Read keys (using %s)", qtls.CipherSuiteTLS13ByID(suite.ID))
		if h.tracer != nil {
	...
	case qtls.EncryptionHandshake:
		...
		h.logger.Debugf("Installed Handshake Read keys (using %s)", qtls.CipherSuiteTLS13ByID(suite.ID))
	case qtls.EncryptionApplication:
		h.readEncLevel = protocol.Encryption1RTT
		h.aead.SetReadKey(suite, trafficSecret)
		h.has1RTTOpener = true
		h.logger.Debugf("Installed 1-RTT Read keys (using %s)", qtls.CipherSuiteTLS13ByID(suite.ID))
	default:
...
func (h *cryptoSetup) SetWriteKey(encLevel qtls.EncryptionLevel, suite *qtls.CipherSuiteTLS13, trafficSecret []byte) {
	...
	case qtls.Encryption0RTT:
		...
		h.mutex.Unlock()
		h.logger.Debugf("Installed 0-RTT Write keys (using %s)", qtls.CipherSuiteTLS13ByID(suite.ID))
		if h.tracer != nil {
	...
	case qtls.EncryptionHandshake:
		...
		h.logger.Debugf("Installed Handshake Write keys (using %s)", qtls.CipherSuiteTLS13ByID(suite.ID))
	case qtls.EncryptionApplication:
		h.writeEncLevel = protocol.Encryption1RTT
		h.aead.SetWriteKey(suite, trafficSecret)
		h.has1RTTSealer = true
		h.logger.Debugf("Installed 1-RTT Write keys (using %s)", qtls.CipherSuiteTLS13ByID(suite.ID))
